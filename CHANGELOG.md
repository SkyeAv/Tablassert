...
### Added
- **`build-kg` now uses an automatic ephemeral shared-prefix TCode cache when guarded expensive prefixes are shared by at least two sections; trivial or unshared prefixes are skipped, and snapshots are cleaned up after the build, including failures.**

### Changed
- **BREAKING: `build-fullmap --aria2c` / `-a` is removed; the downloader is now chosen automatically.** Whenever the optional `[aria2]` extra is installed, both the prebuilt archive download and the BABEL file downloads use the bundled aria2c binary; without the extra (including on macOS, where it ships no wheels), Tablassert's Python downloader is used. The choice is resolved once per run, after the reuse short-circuit, and one info-level log line states it either way, echoed on stderr so the choice is also visible in a normal terminal run (loguru's console sink exists only inside a pipeline stage, and file logging needs the `[log]` extra) — the old exit-2 missing-extra preflight is gone, so a missing extra is no longer an error. Scripts passing `--aria2c` or `-a` must drop the flag — parsing now fails with an unknown-option error. Failure semantics are unchanged: a failed prebuilt download still falls back to a from-scratch BABEL build with a warning (`PrebuiltFullmapUnavailable`), and a failing aria2c download still fails loud (`BabelDownloadError`) instead of being silently rescued by the Python downloader.
