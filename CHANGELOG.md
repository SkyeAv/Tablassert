...
### Added
- **`tablassert quick-map` inspects fullmap entity resolution for one or more terms before a build.** Each term runs the exact build op chain (level-one/level-two normalization, probe-key extraction, redb fetch, filter_and_rank) as one batched redb round trip, and one rich table per term renders in input order with the normalized probe keys in the title so a miss is diagnosable. Flags mirror the NodeEncoding config fields one-to-one (`--taxon` defaulting to 9606, `--prioritize`, `--avoid`, `--exclude-prefixes`, `--exclude-regex`); category values are validated at run time against the live vocabulary, with invalid input exiting 2 and naming the nearest valid names. A miss prints "no matches" and exits 0; only usage errors and an unreadable fullmap exit 2. The core lives in `fullmap.quick_map()`, importable without the `[cli]` extra. ([#191](https://github.com/SkyeAv/Tablassert/pull/191))
- **`Predicates` now carries the DAKP prevention family ahead of the pinned model via `PREDICATE_OVERRIDES`.** `prevents`, `applied_to_prevent`, and `contraindicated_in_the_prevention_of` are deliberate local extensions of the model-derived predicate vocabulary: each emits as its ordinary `biolink:` CURIE and validates as a config `statement.predicate` today, while remaining absent from the pinned biolink-model 4.4.4 `related to` hierarchy (DAKP's assertion pipeline already defers the first two for exactly this reason). Association classes with open-`str` predicate slots -- including the `EntityToDiseaseAssociation` / `EntityToPhenotypicFeatureAssociation` classes DAKP pins via `category_override` -- accept the predicates natively; constrained predicate vocabularies (e.g. `ChemicalEntityToBiologicalProcessAssociation`'s exhaustive `Literal`, the `GeneToDiseasePredicateEnum` family) reject them, and `validate_kgx`'s pending score (`valid_excluding_pending`) now forgives exactly those `predicate: literal_error` / `predicate: enum` failures for records whose own predicate is an override member, while strict `valid` stays strict and unlisted predicates fail both scores. A tripwire test asserts every member is still absent from installed `related to` slots: the moment a biolink-model release adopts one, the suite fails and the stale name is removed, restoring fully model-derived vocabulary for that predicate. No DAKP config changes; downstream can author the new predicates through the existing `statement.predicate` field. ([#193](https://github.com/SkyeAv/Tablassert/pull/193))

### Changed
- **The README PyPI downloads badge now serves from pepy instead of shields.** pepy.tech serves pre-rendered SVGs from its own CDN and does not rate-limit like the shields PyPI endpoint, which had been intermittently failing. ([#190](https://github.com/SkyeAv/Tablassert/pull/190), [#192](https://github.com/SkyeAv/Tablassert/pull/192))
...
### Breaking Changes
- **The `tablassert` command now requires the optional `[cli]` extra.** `cyclopts` and `rich` no longer install with the base Python API; install `pip install "tablassert[cli]"` or `uv tool install "tablassert[cli]"` to use the console command. The launcher preflights the extra before importing the application and reports the exact install command when it is absent. ([#187](https://github.com/SkyeAv/Tablassert/pull/187))

  **Migration:** add the `[cli]` extra to every install that invokes the `tablassert` console command (`pip install "tablassert[cli]"`, `uv tool install "tablassert[cli]"`, or the equivalent dependency pin). Python-API-only consumers need no change and now install less.
- **Level-one normalization now defines fullmap keys by cleaned Unicode-lowercase, whitespace-tokenized, Porter2-normalized terms, with duplicate removal and byte-wise token ordering.** Existing fullmap databases are schema-v5 and are rejected by the current resolver; rebuild fullmaps before use. Preferred-name ranking now compares normalized forms under the same level-one semantics. ([#171](https://github.com/SkyeAv/Tablassert/pull/171), [#172](https://github.com/SkyeAv/Tablassert/pull/172), [#174](https://github.com/SkyeAv/Tablassert/pull/174), [#175](https://github.com/SkyeAv/Tablassert/pull/175), [#176](https://github.com/SkyeAv/Tablassert/pull/176))

  **Migration:** rebuild every fullmap database with `tablassert build-fullmap` (or re-download the prebuilt archive). A schema-v5 file is rejected rather than silently misread, so no build can run against a stale database.
- **`build-fullmap --aria2c` / `-a` is removed; the downloader is now chosen automatically.** Whenever the optional `[aria2]` extra is installed, both the prebuilt archive download and the BABEL file downloads use the bundled aria2c binary; without the extra (including on macOS, where it ships no wheels), Tablassert's Python downloader is used. The choice is resolved once per run, after the reuse short-circuit, and one info-level log line states it either way, echoed on stderr so the choice is also visible in a normal terminal run (loguru's console sink exists only inside a pipeline stage, and file logging needs the `[log]` extra) -- the old exit-2 missing-extra preflight is gone, so a missing extra is no longer an error. Failure semantics are unchanged: a failed prebuilt download still falls back to a from-scratch BABEL build with a warning (`PrebuiltFullmapUnavailable`), and a failing aria2c download still fails loud (`BabelDownloadError`) instead of being silently rescued by the Python downloader. ([#178](https://github.com/SkyeAv/Tablassert/pull/178))

  **Migration:** drop `--aria2c` / `-a` from every invocation -- parsing now fails with an unknown-option error. To keep using aria2c, install the `[aria2]` extra; to stop using it, uninstall the extra.
- **`build-fullmap` always applies the built-in top-100 experimental-taxon allowlist, and `--taxon-allowlist` is removed.** The checked-in `src/tablassert/data/experimental_taxa.yaml` filter is no longer opt-in: every source build passes those 100 NCBI taxon IDs to Rust, and the resulting database records the same deterministic `META.taxon_allowlist` identity introduced in 18.1.0. The identity now also guards both reuse paths, so an unfiltered database can never satisfy an allowlisted `build-fullmap` invocation. A prebuilt archive whose recorded identity is absent or different fails extraction validation before anything is renamed, surfacing as the existing `PrebuiltFullmapUnavailable` fallback to a filtered BABEL build; an existing file at `--output` is reused only when its identity matches, and is otherwise rebuilt with a warning. `build_fullmap_pipeline`, `fetch_prebuilt_fullmap`, and `_extract_prebuilt_fullmap` accept the IDs through their existing/extended `taxon_allowlist` parameter, and the Rust extension exposes `taxon_allowlist_identity` plus `fullmap_taxon_allowlist_identity` so Python can compare identities without duplicating the encoding. ([#153](https://github.com/SkyeAv/Tablassert/pull/153))

  **Migration:** drop `--taxon-allowlist` / `--no-taxon-allowlist` -- parsing now fails with an unknown-option error. Unfiltered databases built by earlier releases no longer satisfy a reuse check and are rebuilt with a warning; budget for one filtered rebuild.
- **Logging now requires the optional `log` extra and is completely disabled without it.** The stdlib-backed loguru fallback, the startup missing-extra warning, and the base-install creation of `.tablassert/log/` are removed. Installs without loguru emit no file, progress, or warning logs. ([#152](https://github.com/SkyeAv/Tablassert/pull/152))

  **Migration:** install `pip install "tablassert[log]"` anywhere logs are relied on; a base install is now silent rather than degraded.

### Added
- **DAKP sparse qualifier stack now survives `prune_to_class` on the pinned classes.** `CLASS_FIELD_OVERRIDES` grants `anatomical_context_qualifier`, `sex_qualifier`, `population_context_qualifier`, `frequency_qualifier`, and `temporal_context_qualifier` to `EntityToDiseaseAssociation` / `EntityToPhenotypicFeatureAssociation` alongside the existing `disease_context_qualifier` / `regulatory_approvals` grants, so DAKP assertion rows carrying those qualifiers keep them on the edge instead of having them nulled and rescued into the pruned column. Deliberately ungranted: `species_context_qualifier` (disabled -- never emittable) plus `temporal_interval_qualifier` and `severity_qualifier` (unsatisfiable -- attached to no class, so a grant could never validate). Record validation already strips granted fields from its in-memory copy, so no new `extra_forbidden` surface appears; the existing tripwire test fails the suite the moment a biolink-model release attaches a granted slot, until the stale grant is removed. ([#188](https://github.com/SkyeAv/Tablassert/pull/188))
- **`tablassert.net` is the new stdlib-only network-resilience seam shared by the agent and the BABEL downloader.** It classifies transient versus permanent failures through one ordered table (DNS/connection/timeout failures, HTTP 408/425/429 and every 5xx, and rate-limit/quota messages are transient; other 4xx and local `OSError`s such as `ENOSPC` fail fast), retries transient failures with bounded jittered backoff -- 4 attempts, 1.0 s base doubling to 20.0 s, 60 s total -- and honors `Retry-After` headers and `(reset after ...)` quota hints (which set the wait, capped at 60 s). The agent's article fetches (`_http_get_text` / `_http_get_bytes`) route through it, so a resolver blip no longer terminal-skips an article: exhausted retries raise a coded `network-transient` error instead. ([#163](https://github.com/SkyeAv/Tablassert/pull/163), [#164](https://github.com/SkyeAv/Tablassert/pull/164))
- **Agent LLM calls now use one bounded, jittered retry layer across the inner agent, reflexion, and judge.** Smolagents' long rate-limit retryer and the OpenAI client's transport retries are disabled, while the shared seam retries DNS, timeouts, 429, and 5xx failures with Retry-After and a 45-second per-call budget; LiteLLM's backend-internal retry remains outside its constructor controls. The worst-case 29 logical calls per article add at most 21.8 minutes, and exhausted calls persist `llm-transient` without exposing configured credentials in logs or state. ([#167](https://github.com/SkyeAv/Tablassert/pull/167))
- **Agent checkpoint records now expose machine-readable `error_code` values for skipped articles.** `network-transient` and `llm-transient` identify outcomes that a fleet consumer should requeue; deterministic gates and pre-field state files remain `null`. The supervisor also emits one visible `ERROR` log line for every catch-all skip while preserving the existing `SKIPPED` status and notes prefix for mixed-version consumers. ([#166](https://github.com/SkyeAv/Tablassert/pull/166))
- **`build-kg` now uses an automatic ephemeral shared-prefix TCode cache when guarded expensive prefixes are shared by at least two sections.** Content-addressed XXH64 op digests key each prefix, worth-caching guards (`OP_COST` plus checkpoint boundaries) keep trivial or unshared prefixes out of the cache, the planner reorders section execution so shared prefixes are computed once, and the parquet snapshots live in a temp dir that is cleaned up after the build, including on failure. The final write reuses the producer's snapshot rather than recollecting it. ([#155](https://github.com/SkyeAv/Tablassert/pull/155), [#156](https://github.com/SkyeAv/Tablassert/pull/156), [#157](https://github.com/SkyeAv/Tablassert/pull/157), [#158](https://github.com/SkyeAv/Tablassert/pull/158), [#159](https://github.com/SkyeAv/Tablassert/pull/159), [#161](https://github.com/SkyeAv/Tablassert/pull/161))
- **The `--distill` corpus is now schema-uniform, carries each run's outcome, and can be turned into a weighted SFT dataset.** Every recorded line emits the full canonical key set with explicit nulls (v2 adds `record_type`, `schema_version`, and a `run_id` join key), because `datasets.load_dataset("json")` infers features from the first block of the first file and raises `CastError` when a later block of an append-only corpus introduces a new column. A sibling `outcomes.ndjson` receives one 35-key outcome line per supervisor run -- captured at every terminal exit including the two validate-gate skips, the derive-mode exit, and the crash path, so failed runs persist as negative examples -- assembled by the new pure `src/tablassert/distill_reward.py` from data the supervisor already holds, with no extra build, audit, or KGX read. That module also scores each run with a deterministic reward clamped to [0, 1] (0.40 coverage, 0.28 Biolink validity, 0.17 specificity, 0.07 tool-call cleanliness, 0.08 breadth, with multiplicative hard gates and farming penalties) whose coefficients are tunable through a YAML/JSON `RewardConfig` that fails loud with the coded `reward-config-invalid`. The new `tablassert distill-weigh` composes join, reward, and selection (`threshold` / `best-of-n` / `replication`) into one row per input record plus a deterministic manifest, and is zero-dependency (stdlib plus PyYAML, no `[distill]` extra). This is data selection for supervised LoRA/QLoRA fine-tuning and explicitly not RLHF: no reward model, no PPO/GRPO/DPO/KTO, and no trainer import, enforced by an executable source-scan test. ([#180](https://github.com/SkyeAv/Tablassert/pull/180), [#181](https://github.com/SkyeAv/Tablassert/pull/181), [#182](https://github.com/SkyeAv/Tablassert/pull/182), [#183](https://github.com/SkyeAv/Tablassert/pull/183))

### Fixed
- **`distill-export` no longer breaks on an append-only corpus whose files disagree in shape.** `datasets.load_dataset("json")` infers its column schema from the first block of the first file only and casts every later table to it, so a directory holding both v1 and v2 records either raised `CastError` or silently JSON-encoded a varying-type column into a string, and outcome files sitting alongside records made a record-shaped corpus unexportable. Export now partitions the directory by content (`is_outcome_file` reads the first non-blank line's `record_type`, so a renamed file still partitions correctly), unions the schema across every record file into one uniform temporary NDJSON written outside `--distill-dir` and always removed, and rejects genuine type conflicts with exit 2 naming the key and types before the `[distill]` preflight and the datasets import. Record files containing zero rows now fail loud instead of surfacing a bare `StopIteration` from inside the datasets JSON reader. Flags, defaults, exit codes, and preflight ordering are unchanged. ([#184](https://github.com/SkyeAv/Tablassert/pull/184))
- **BABEL downloads now classify transient failures through one shared table instead of an inline copy.** `download_babel_file`'s hand-rolled non-retryable predicate (`cli.py`) is replaced by `tablassert.net.is_transient`, the new stdlib-only network-resilience seam. Three classification deltas follow, all intentional and pinned by tests: HTTP 425 Too Early is now retried instead of failing fast (the old rule listed only 408 and 429, though RFC 9110 defines 425 as "try again"); a genuinely permanent local `OSError` such as `ENOSPC` on the `.part` file now fails fast instead of burning all five attempts and up to 75 s of backoff; and a bare mid-stream `OSError` carrying a network errno (`ENETUNREACH`, `ECONNRESET`, `EPIPE`, `EMFILE`, ...) keeps the full five-attempt guarantee it always had, now via the errno table rather than the old catch-all clause. The Range resume, streaming, progress callback, `retries=5` default, and `min(60, 5 * 2 ** (attempt - 1))` schedule are unchanged. ([#163](https://github.com/SkyeAv/Tablassert/pull/163))
- **BABEL download retry warnings now survive the loguru sink.** The retry-warning log bound the raw exception object, which the `enqueue=True` sink cannot pickle (`HTTPError.__init__` requires five positional arguments), so every BABEL retry warning line was silently dropped with an internal `--- End of logging error ---`. It now binds the stringified error, and the same hazard was fixed on the new `NET` retry logging at its seam. ([#163](https://github.com/SkyeAv/Tablassert/pull/163))

### Performance
- **PMC article downloads are idempotent, atomic, and bounded-parallel.** A file that already exists non-empty is skipped without any request (a torn zero-length file re-downloads), each download writes a `.part` sibling that is atomically `os.replace`d into place on completion — and removed even on `KeyboardInterrupt`, so no half-written file survives — and up to 8 files fetch concurrently by default with results collected in submission order. `concurrency < 1` now fails before any network call is made. Previously every file downloaded sequentially, single-attempt, on every rerun. ([#165](https://github.com/SkyeAv/Tablassert/pull/165))
- **Reflexion and judge prompts are hard-bounded to fit a context window.** The reflexion prompt caps its current-config, coverage-report, and article-context blocks at 8,000 / 8,000 / 40,000 chars and the judge prompt caps its serialized report at 8,000 chars; every `unresolved` term list keeps 20 entries plus a visible `+N more` marker and an `unresolved_count` naming the original length. The worst-case prompt is ≈ 58,000 chars ≈ 15,000 tokens — the fleet distill telemetry that motivated this recorded 1.6M-character / 689,241-input-token reflexion prompts (a ≥ 27× reduction). Public coverage and audit reports are unchanged; compaction happens only at prompt serialization. ([#168](https://github.com/SkyeAv/Tablassert/pull/168))

### Documentation
- **`docs/agent.md` now documents the network-resilience layer, `error_code`, and the prompt bounds.** A new `## Network resilience` section covers the transient/permanent classification table, the HTTP and LLM retry budgets, `Retry-After` honoring with its 60 s cap, idempotent atomic parallel downloads, the worst-case wall-clock arithmetic against the 90-minute per-article budget, and the single-retry-layer rule; the checkpoint prose documents `error_code` values and requeue semantics; the reflexion/judge prose documents the prompt-bound constants and the fleet evidence behind them. `llms.txt` indexes the new `net.py` module. ([#169](https://github.com/SkyeAv/Tablassert/pull/169))
- **`docs/agent.md`'s `--distill` section now documents the v2 corpus, the reward, and the LoRA workflow.** It covers the 13-key record line and the sibling 35-key outcome line with `run_id` as the join key, every `distill-weigh` flag and default, the reward's five additive terms with their hard gates and farming penalties cross-checked against the live `RewardConfig` defaults, a three-row selection-policy table with caveats, and the consumption limits (TRL `max_length` truncation and fully-masked drops, packing making weights token-proportional). It states plainly that the goal is LoRA/QLoRA SFT and not RLHF, and records the honest limits with citations rather than overclaiming generalization. ([#185](https://github.com/SkyeAv/Tablassert/pull/185))
- **`docs/cli.md` documents `distill-weigh` and the hardened `distill-export`.** The command table, the full per-flag section, the manifest contents, and `llms.txt` now match the shipped CLI. ([#183](https://github.com/SkyeAv/Tablassert/pull/183), [#184](https://github.com/SkyeAv/Tablassert/pull/184))
- **`docs/api/fullmap.md` text matches the new level-one normalization.** ([#176](https://github.com/SkyeAv/Tablassert/pull/176))

### Changed
- **`README.md` no longer shows the PyPI monthly downloads badge.** The remaining PyPI, Python, CI, License, Docs, and stars badges are untouched. ([#154](https://github.com/SkyeAv/Tablassert/pull/154))

## 18.1.0 - 2026-09-14

### Added
- **`build-fullmap --taxon-allowlist` builds an opt-in, source-derived fullmap restricted to the top 100 experimental taxa, and InChIKey terms now survive ingestion.** The flag reads the checked-in `src/tablassert/data/experimental_taxa.yaml` allowlist (100 NCBI taxon IDs ranked by observed frequency across the `MultiomicsNext` and `MultiomicsHarness` corpora) and filters non-`OrganismTaxon` synonym rows carrying valid taxon metadata in `rust/src/fullmap.rs` before interning, yielding a smaller database while retaining taxonless rows (including `NCBITaxon:0` and malformed taxon values), rows with any allowlisted taxon, and every row whose categories include `OrganismTaxon`; the first taxon is still the one stored, so hydration and lookup behavior are unchanged. Filtered builds always come from BABEL sources, never reuse the unfiltered prebuilt archive, and record a deterministic `META.taxon_allowlist` identity so a filtered database cannot be mistaken for a full one. Separately, Rust token QC no longer rejects `inchikey` -- newline, tab, hypothetical, uncharacterized, and dead-token filtering are all preserved -- so InChIKey terms are indexed and resolvable in both raw normalized (`abc-def`) and CURIE (`inchikey:abc-def`) form. Default fullmap builds, the prebuilt download path, and existing Python callers are untouched. ([#149](https://github.com/SkyeAv/Tablassert/pull/149))

### Fixed
- **The `Categories` config vocabulary now names fullmap-emitted Biolink mixins via `CATEGORY_OVERRIDES`.** Biolink declares some node categories as mixins, and the pydantic model generates a mixin as a plain `ConfiguredBaseModel` that is not an `Entity` subclass -- invisible to the scan that builds `Categories`. The fullmap's BABEL vocabulary assigns one such mixin (`GenomicEntity`) to real terms, so an `avoid` allow-list could never name it: the category sailed through the guard and the edge fell back to bare `biolink:Association`, whose `prune_to_class` then nulled every class-specific slot into the inlined `has_supporting_studies` junk drawer (DAKP shipped 35 such edges). `GenomicEntity` is now a first-class member -- valid in `avoid`/`prioritize` and as a `category_override` key -- and a tripwire test drops the override the moment the installed model makes the mixin an `Entity` subclass (tests in `tests/test_biolink.py` and `tests/test_models.py`). ([#151](https://github.com/SkyeAv/Tablassert/pull/151))
- **`filter_and_rank` drops categories the `Categories` enum cannot name when `avoid` is set.** An `avoid` list is an allow-list by complement, and a complement can only name what the enum knows: a fullmap `CATEGORY_NAME` outside the enum could never appear in it, so without this guard the hard filter had a silent hole for any future vocabulary drift. Degradation is now to lost rows (surfaced in the resolution report) rather than mis-classed edges (regression tests in `tests/test_fullmap.py`). ([#151](https://github.com/SkyeAv/Tablassert/pull/151))
- **The `--qc` study stage now fails the build on edges demoted to bare `biolink:Association` when any section declares a `category_override`.** A pinned section promises every row lands on a pinned class, so a demotion means a row escaped the pin -- typically an object category the config vocabulary could not name -- and shipped without its class-specific evidence slots. Without a pin, bare `Association` remains the author's accepted default, so the assertion is gated. `study_kgx` gained the `category_override_declared` keyword; `study_final_ndjson` forwards it from the built sections (regression tests in `tests/test_study.py`). ([#151](https://github.com/SkyeAv/Tablassert/pull/151))
